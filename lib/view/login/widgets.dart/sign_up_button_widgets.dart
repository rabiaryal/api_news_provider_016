  ...
  Widget build(BuildContext context) {
    return Consumer<AuthViewModel>(
      builder: (context, provider, child) {
        return RoundButton(
          title: 'Sign Up',
          loading: provider.isLoading, // Using the loading state from AuthViewModel
          onPress: () async {
            ...
            if (provider.email.isEmpty) {
              ...
              Utils.flushBarErrorMessage('Password must be at least 6 characters long', context);
            } else {
             
              try {
                await provider.register(context); // Call login from AuthViewModel
